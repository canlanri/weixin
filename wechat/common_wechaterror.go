...
func (err *WechatErr) GetDesc() string {
	if desc, ok := errDesc[err.ErrCode]; ok {
		return desc
	}
	return ""
}

func (err *WechatErr) Error() string {
	return fmt.Sprintf("errcode: %d, errmsg: %s, errdesc: %s", err.ErrCode, err.ErrMsg, err.GetDesc())
}
